...
export const GoogleReviewGate: React.FC<GoogleReviewGateProps> = ({
  ...
  return (
    <motion.div
      ...
      className="flex flex-col items-center text-center p-6 sm:p-8 bg-slate-900/95 backdrop-blur-2xl rounded-3xl sm:rounded-4xl shadow-2xl border border-slate-800 max-w-md w-full mx-auto relative overflow-hidden"
    >
      {/* Background radial brand glow */}
      <div
        className="absolute -inset-10 rounded-full blur-3xl opacity-20 pointer-events-none"
        style={{ backgroundColor: primaryColor }}
      />

      {/* 5 Stars Rating Badge */}
      <div className="flex items-center gap-1.5 bg-amber-500/15 px-4 py-1.5 rounded-full mb-4 border border-amber-500/30">
        {Array.from({ length: rating }).map((_, i) => (
          <Star key={i} className="w-4 h-4 fill-amber-400 text-amber-400" />
        ))}
        <span className="text-amber-300 text-xs font-black ml-1">{rating}/5 Confirmé</span>
      </div>
      ...
      {status === 'initial' && (
        ...
        >
          <p className="text-slate-300 text-sm leading-relaxed max-w-xs mx-auto">
            ...
            <strong className="text-white">Partagez votre expérience sur Google</strong> pour débloquer immédiatement l'accès à la <strong className="text-amber-400">Roue des Cadeaux</strong> !
          </p>
        ...
        >
          ...
          {isTabReturned && (
            <motion.div
              ...
              animate={{ opacity: 1, y: 0 }}
              className="p-2.5 bg-amber-500/15 border border-amber-500/30 rounded-xl text-amber-300 text-xs font-semibold flex items-center gap-2"
            >
          ...
          <button
            disabled
            className="w-full flex items-center justify-center gap-2 py-4 px-6 rounded-2xl bg-slate-800 text-slate-400 font-bold text-sm border border-slate-700 cursor-not-allowed"
          >
      ...
      {status === 'unlocked' && (
        ...
        >
          ...
          <div className="p-3.5 bg-emerald-500/15 border border-emerald-500/30 rounded-2xl text-emerald-300 text-xs font-bold flex items-center justify-center gap-2 shadow-inner">
          ...
          <motion.button
            ...
            className="w-full flex items-center justify-center gap-3 py-4 px-6 rounded-2xl text-white font-black text-base shadow-2xl transition-all cursor-pointer animate-pulse"
            style={{
